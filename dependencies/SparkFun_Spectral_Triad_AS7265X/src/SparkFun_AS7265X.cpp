...
namespace
{
  // The virtual-register bridge normally responds within a few milliseconds. Bound each state
  // poll so a missing or wedged sensor cannot hold the SAMD21 forever.
  constexpr uint32_t AS7265X_VIRTUAL_REGISTER_TIMEOUT_MS = 100;
}
#include "Arduino.h"
...
uint8_t AS7265X::virtualReadRegister(uint8_t virtualAddr)
{
  ...
  while (1)
  {
    ...
    if (millis() - waitStart >= AS7265X_VIRTUAL_REGISTER_TIMEOUT_MS)
      return (0);
    delay(AS7265X_POLLING_DELAY);
  }
