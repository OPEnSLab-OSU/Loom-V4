...
namespace {
// A normal 100 kHz byte transaction completes in well under a millisecond. Allow substantial
// clock-stretching margin, but never let a wedged I2C bus halt the SAMD21 forever.
constexpr uint32_t WIRE_OPERATION_TIMEOUT_MS = 100;
}
...
void SERCOM::prepareCommandBitsWire(uint8_t cmd)
{
  if(isMasterWIRE()) {
    ...
    {
      if (millis() - waitStart >= WIRE_OPERATION_TIMEOUT_MS)
        return;
    }
  } else {
...
bool SERCOM::startTransmissionWIRE(uint8_t address, SercomWireReadWriteFlag flag)
{
  ...
  // Do not deadlock if another master owns the bus or a slave left it wedged. This matches the
  // behavior of newer Arduino/Adafruit SAMD cores: the caller receives a failed transaction and
  // can retry after its normal sensor power-cycle path.
  if (!isBusIdleWIRE() && !isBusOwnerWIRE())
    return false;
  ...
  if ( flag == WIRE_WRITE_FLAG ) // Write mode
  {
    ...
    while( !sercom->I2CM.INTFLAG.bit.MB )
    {
      if (sercom->I2CM.INTFLAG.bit.ERROR ||
          millis() - waitStart >= WIRE_OPERATION_TIMEOUT_MS)
        return false;
    }
  }
  else  // Read mode
