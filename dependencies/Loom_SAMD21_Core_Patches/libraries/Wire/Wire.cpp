...
namespace {
constexpr uint32_t WIRE_READ_TIMEOUT_MS = 100;
}
...
uint8_t TwoWire::requestFrom(uint8_t address, size_t quantity, bool stopBit)
{
  ...
  {
    // Read first data
    if (!sercom->availableWIRE())
      return 0;
    const uint8_t firstByte = sercom->readDataWIRE();
    if (!sercom->isBusOwnerWIRE())
      return 0;
    rxBuffer.store_char(firstByte);
    byteRead = 1;

    // Connected to slave
    while (byteRead < quantity && sercom->isBusOwnerWIRE())
    {
      sercom->prepareAckBitWIRE();                          // Prepare Acknowledge
      sercom->prepareCommandBitsWire(WIRE_MASTER_ACT_READ); // Prepare the ACK command for the slave

      const uint32_t waitStart = millis();
      while (!sercom->availableWIRE() && sercom->isBusOwnerWIRE() &&
             millis() - waitStart < WIRE_READ_TIMEOUT_MS)
      {
      }
      if (!sercom->availableWIRE())
        break;

      const uint8_t nextByte = sercom->readDataWIRE();      // Read data and send the ACK
      if (!sercom->isBusOwnerWIRE())
        break;
      rxBuffer.store_char(nextByte);
      ++byteRead;
    }
    sercom->prepareNackBitWIRE();                           // Prepare NACK to stop slave transmission
    //sercom->readDataWIRE();                               // Clear data register to send NACK

    if (stopBit && sercom->isBusOwnerWIRE())
    {
