...
bool ds3231WriteRegister(uint8_t reg, uint8_t value) { return ds3231Write(reg, &value, 1); }
...
bool ds3231Read(uint8_t startRegister, uint8_t *data, size_t length) {
    ...
    const size_t received =
        Wire.requestFrom(static_cast<uint8_t>(DS3231_ADDRESS), static_cast<uint8_t>(length));
    if (received != length) {
...
bool ds3231ReadRegister(uint8_t reg, uint8_t &value) { return ds3231Read(reg, &value, 1); }
...
char *DateTime::text(void) const { return text(legacyDateTimeText, sizeof(legacyDateTimeText)); }

char *DateTime::text(char *buffer, size_t length) const {
    if (buffer == nullptr || length == 0)
        return buffer;
    snprintf(buffer, length, "%04u.%02u.%02u %02u:%02u:%02u", static_cast<unsigned int>(year()),
             static_cast<unsigned int>(month()), static_cast<unsigned int>(day()),
             static_cast<unsigned int>(hour()), static_cast<unsigned int>(minute()),
...
void PCF8523::write_reg(uint8_t address, uint8_t data) { write_reg(address, &data, 1); }
...
// Reset the PCF8523
void PCF8523::reset() { write_reg(PCF8523_CONTROL_1, 0x58); }

uint8_t PCF8523::clear_rtc_interrupt_flags() {
    uint8_t rc2 =
        read_reg(PCF8523_CONTROL_2) & (PCF8523_CONTROL_2_SF_BIT | PCF8523_CONTROL_2_AF_BIT);
...
uint8_t PCF8523::getTimer1(void) { return read_reg(PCF8523_TMR_A_REG); }
...
uint8_t PCF8523::getTimer2(void) { return read_reg(PCF8523_TMR_B_REG); }
...
void RTC_Millis::adjust(const DateTime &dt) { offset = dt.unixtime() - millis() / 1000; }
...
bool RTC_DS3231::adjustChecked(const DateTime &dt) {
    ...
    const uint8_t values[7] = {
        ...
        bin2bcd(dt.dayOfTheWeek() + 1), bin2bcd(dt.day()),    bin2bcd(dt.month()),
...
bool RTC_DS3231::setAlarm(Ds3231_ALARM_TYPES_t alarmType, byte seconds, byte minutes, byte hours,
                          byte daydate) {
    ...
    const bool written =
        alarmTwo ? ds3231Write(ALM2_MINUTES, values + 1, 3) : ds3231Write(ALM1_SECONDS, values, 4);
    if (!written)
        return false;
    return armAlarm(alarmTwo ? 2 : 1, true);
}

bool RTC_DS3231::setAlarm(Ds3231_ALARM_TYPES_t alarmType, byte minutes, byte hours, byte daydate) {
...
bool RTC_DS3231::setAlarm1(const DateTime &t, Ds3231Alarm1Mode alarmMode) {
    ...
    return setAlarm(static_cast<Ds3231_ALARM_TYPES_t>(alarmMode), t.second(), t.minute(), t.hour(),
                    dayOrDate);
}
