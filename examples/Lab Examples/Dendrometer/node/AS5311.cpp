 ...
 * @return 32 bit value, of which the 18 least signifcant bits contain the
 * sensor data
 */
uint32_t AS5311::bitbang(bool angleData = true) {
  ...
  if (angleData) {
    digitalWrite(CLK_PIN,
                 HIGH); // write clock high to select the angular position data
  } else {
    digitalWrite(
        CLK_PIN,
        LOW); // write clock high to select the magnetic field strength data
  }
...
/**
 * Record the data from the magnet sensor, process it, and add it to the
 * manager's packet.
 */
 ...
 * Moving the sensor too much (about 1mm) in between calls to this function will
...
float AS5311::measureDisplacement(int pos) {
  ...
  return ((magnetPosition - initialPosition) * UM_PER_TICK) +
