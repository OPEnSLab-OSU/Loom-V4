...
if "%SAVE_REPORT%"=="1" (
    ...
    if defined CPP_EXTRA_FLAGS >>"%SUMMARY_TXT%" echo C++ extra flags: %CPP_EXTRA_FLAGS%
    if defined SKETCH_LIST_FILE >>"%SUMMARY_TXT%" echo Requested sketch list: %SKETCH_LIST_FILE%
...
if defined CPP_EXTRA_FLAGS echo C++ extra flags: %CPP_EXTRA_FLAGS%
if defined SKETCH_LIST_FILE echo Requested sketch list: %SKETCH_LIST_FILE%
...
if defined SKETCH_LIST_FILE (
    if not exist "%SKETCH_LIST_FILE%" (
        call :Red "ERROR: Requested sketch list was not found: %SKETCH_LIST_FILE%"
        call :Cleanup
        call :MaybePause
        exit /b 1
    )
    set "SKETCH_LIST_SNAPSHOT=%OUT_ROOT%\requested_sketches.txt"
    copy /y "%SKETCH_LIST_FILE%" "!SKETCH_LIST_SNAPSHOT!" >nul
    for /f "usebackq delims=" %%D in ("!SKETCH_LIST_SNAPSHOT!") do (
        echo %%~fD>>"%RAW_DIRS%"
    )
) else (
...
for /f "usebackq delims=" %%D in ("%SKETCH_DIRS%") do (
    ...
    set "ACTIVE_CPP_EXTRA_FLAGS=!CPP_EXTRA_FLAGS!"
    ...
    if defined ACTIVE_CPP_EXTRA_FLAGS (
        if exist "%SKETCHBOOK_LIBS%" (
            "%ARDUINO_CLI%" compile --fqbn "%FQBN%" --warnings "%WARNINGS%" --jobs 0 --build-property "compiler.cpp.extra_flags=!ACTIVE_CPP_EXTRA_FLAGS!" --libraries "%PACKAGE_LIBS%" --libraries "%SKETCHBOOK_LIBS%" --build-path "!BUILD_DIR!" . > "!LOG_FILE!" 2>&1
        ) else (
            "%ARDUINO_CLI%" compile --fqbn "%FQBN%" --warnings "%WARNINGS%" --jobs 0 --build-property "compiler.cpp.extra_flags=!ACTIVE_CPP_EXTRA_FLAGS!" --libraries "%PACKAGE_LIBS%" --build-path "!BUILD_DIR!" . > "!LOG_FILE!" 2>&1
        )
    ) else (
        if exist "%SKETCHBOOK_LIBS%" (
