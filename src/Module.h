...
public:
  ...
  // Generic measure and package calls to unify some interaction with different
  ...
  }; // Called by the manager to allow OLED to display data at the same time as
     // manager.display_data

  bool moduleInitialized = true; // Whether or not the module initialized
                                 // successfully true until set otherwise
  int module_address = -1;       // Specifically for I2C addresses, -1 means the
                                 // module doesn't have an address
private:
  char moduleName[100];
};