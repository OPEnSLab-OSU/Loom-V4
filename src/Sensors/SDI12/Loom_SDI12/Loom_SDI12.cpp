...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_SDI12::Loom_SDI12(const int pinNumber)
    : Module("SDI12"), sdiInterface(pinNumber) {}
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Loom_SDI12::initialize() {
  ...
  // Start the interface and then wait for 100ms to allow things to settle and
  // startup correctly
  sdiInterface.begin();
  delay(100);
  ...
  for (int i = 0; i < inUseAddresses.size(); i++) {
    ...
    addressToType.insert(
        std::pair<char, const char *>(inUseAddresses[i], response));
...
void Loom_SDI12::measure() {
  ...
  // On measure we also want to reset the mode to output in case the 4G board
  // has messed with it
  pinMode(sdiInterface.getDataPin(), OUTPUT);
  delay(30);
...
std::vector<char> Loom_SDI12::scanAddressSpace() {
  ...
  // Scan over the characters that can be used as addresses for refrencing the
  // sensors
  for (char i = '0'; i <= '9'; i++) {
    if (checkActive(i)) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_SDI12::sendCommand(char response[RESPONSE_SIZE], char addr,
                             const char *command) {
  // Send a request to the sensor at the given address and then wait 30ms before
  // continuing
  char output[25];
  memset(output, '\0', 25);
...
void Loom_SDI12::readResponse(char response[RESPONSE_SIZE]) {
  int index = 0;
  memset(response, '\0', RESPONSE_SIZE);
  // While data is available to be read read until an end line character
  // appears.
  while (sdiInterface.available()) {
    char c = sdiInterface.read();

    // Command responses terminate with an endline so we should stop when we see
    // this
    if (c == '\n') {
      response[index] = '\0';
      break;
...
void Loom_SDI12::getData(char addr) {
  ...
  if (strlen(response) > 1) {
    ...
    if (strstr(getSensorInfo(addr), "GS3") != NULL) {
      // Read out the results and parse out each of the data readings and pares
      // them to floats
      p = strtok(response, "+");
