...
/**
 * Provides both a loomified in addition to a standard reliable library
 * implementation
 *
...
public:
  ...
  /* The following methods are intended for manual usage, outside of the Loom
   * framework*/
  ...
  void
  sendCommand(char response[RESPONSE_SIZE], char addr,
              const char *command); // Sends the given command to the given
                                    // sensor on the bus and returns the result
  ...
  std::vector<char>
  scanAddressSpace(); // Scans over the SDI-12 address space and returns a list
                      // of in-use addresses
...
private:
  ...
  int sensorTracker =
      0; // If we have multiple SDI-12 sensors on one bus we need to distinguish
         // them in the json so increment a counter per

  float sensorData[3]; // Array of floats to store sensor data
  std::vector<char>
      inUseAddresses; // List of address that have SDI_12 sensors connected
  std::vector<char *> sensorNames; // List of strings of sensor names

  std::map<char, const char *>
      addressToType; // Maps an SDI12 device address to a device type

  void
  readResponse(char response[RESPONSE_SIZE]); // Reads and returns the sensor's
                                              // response to the command
  bool checkActive(
      char addr); // Checks if the current address is actually being used
};
