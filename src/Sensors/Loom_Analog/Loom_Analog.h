...
struct AnalogMapping {
  ...
  AnalogMapping(int pinNumber, const char *name, float analog,
                float analog_mv) {
...
public:
   ...
   * @param additionalPins Variable length argument allowing you to supply
   * multiple pins
   */
  ...
  Loom_Analog(Manager &man, T firstPin, Args... additionalPins)
    ...
    pinMappings.push_back(new AnalogMapping(A7, "Vbat", getBatteryVoltage(),
                                            getBatteryVoltage() * 1000));
    manInst = &man;
...
private:
  /**
   *   The following two functions are some sorcery to get the variadic
   * parameters without the need for passing in a size variable I don't fully
   * understand it so don't touch it just works Based off:
  ...
  char *
  pinNumberToName(int pin); // Convert the given to a name with the style "A0"

  Manager *manInst; // Instance of the manager
  std::vector<AnalogMapping *>
      pinMappings; // Contains a struct for each pin we are monitoring
};