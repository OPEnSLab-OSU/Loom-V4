...
void Loom_Analog::measure() {
    ...
    for (int i = 0; i < pinMappings.size(); i++) {
        ...
        /* If its a normal pin then just read the value and update the previous
           values */
        else {
...
void Loom_Analog::package() {
    ...
    /* Loop over the list of pins and pull out the data to formulate the JSON
     * entries*/
    for (int i = 0; i < pinMappings.size(); i++) {
        memset(output, '\0', 10);
        json[pinMappings[i]->name] = pinMappings[i]->analog;

        /* Append MV to the name to differentiate between normal analog and the
         * millivolt representation */
