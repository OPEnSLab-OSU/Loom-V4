...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_SEN55::Loom_SEN55(Manager &man, bool measurePM, bool useMux,
                       bool readNumVals)
    : I2CDevice("SEN55"), manInst(&man), measurePM(measurePM),
      readNumVals(readNumVals) {
...
void Loom_SEN55::initialize() {
  ...
  if (error) {
    ...
    snprintf(output, OUTPUT_SIZE,
             "Error occurred while attempting to reset device: %s, module will "
             "not be initialized!",
             errorMessage);
    ERROR(output);
...
void Loom_SEN55::measure() {
  ...
  if (measurePM) {
    LOG(F("Beginning PM measurement, waiting 2 seconds at each increment for "
          "stablizing mesurement..."));
    ...
    for (int i = 0; i < PM_AVERAGE_COUNT; i++) {
      ...
      if (!dataReady && i == 0) {
        ...
        LOG(F("No data available on iteration 0, waiting an additional 5 "
              "seconds to see if data becomes available"));
      ...
      if (dataReady)
        sen5x.readMeasuredPmValues(Pm1p0, Pm2p5, Pm4p0, Pm10p0, numPm0p5,
                                   numPm1p0, numPm2p5, numPm4p0, numPm10p0,
                                   particleSize);
      else {
        failedReads++;
      }
    ...
    if (failedReads < PM_AVERAGE_COUNT) {
      ...
      ERROR("Failed to read any data from the sensor, values might be "
            "incorrect. If this persists, please check for errors...");
    }

    float tmp = 0.0;
    readErr = sen5x.readMeasuredValues(tmp, tmp, tmp, tmp, ambientHumidity,
    ...
    LOG("Beginning measurement without PM, waiting 10 seconds for sensor to "
        "stabilize...");
    ...
    bool dataReady = false;
    uint16_t startTime = millis();
    LOG(F("Waiting for data to be ready... If not ready in 10 seconds we will "
          "stop trying"));
    while (!dataReady && millis() < startTime + 10000) {
      ...
      if (error) {
        ...
        snprintf(output, OUTPUT_SIZE,
                 "Failed to check if data was ready to be read: %s",
                 sensorError);
        ERROR(output);
      }
    }
    ...
    if (dataReady) {
      ...
      if (error) {
        errorToString(error, sensorError, OUTPUT_SIZE);
        snprintf(output, OUTPUT_SIZE,
                 "Error occurred when reading measurement: %s", sensorError);
        ERROR(output);
        FUNCTION_END;
        return;
      }
...
void Loom_SEN55::adjustTempOffset(float offset) {
  ...
  if (moduleInitialized) {
    ...
    if (error) {
      errorToString(error, sensorError, OUTPUT_SIZE);
      snprintf(output, OUTPUT_SIZE, "Failed to adjust sensor offset: %s",
               sensorError);
      ERROR(output);
    }
  }
...
void Loom_SEN55::logDeviceStatus() {
  ...
  if (error) {
    errorToString(error, sensorError, OUTPUT_SIZE);
    snprintf(output, OUTPUT_SIZE,
             "Error occurred while logging device status: %s", sensorError);
    ERROR(output);
  }
