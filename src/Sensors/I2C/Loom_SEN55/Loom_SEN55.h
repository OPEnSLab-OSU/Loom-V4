...
#define PM_AVERAGE_COUNT                                                       \
  10 // Number of times to read the pm values then average them over

/**
 *  SEN55 Air Quality sensors, supports pm 1.0, 2.5, 4.0, 10 as well as
 * Temp/Humidity and Nox and Voc index
 *
 * NOTE: To get accurate results using the SE555 it should be powered on and
 * remain on as according to the data sheet, the switch-on behavior for the VOC
 * Index is ~ 1 hr and the NOx is ~ 6 hours. Data sheet:
 * https://cdn.sparkfun.com/assets/5/b/f/2/8/Sensirion_Datasheet_SEN5x.pdf (Page
 * 8)
 *
...
public:
   ...
   * data
   * @param measurePM This sets whether or not we should conduct measurements
   * without PM readings (uses less power)
   * @param useMux Whether or not we are using the multiplexer class
   */
  Loom_SEN55(Manager &man, bool measurePM = true, bool useMux = false,
             bool numMode = true);

  /**
   * Adjust the temperature reading offset in celsius. By default it accounts
   * for the internal heating of the sensor. Adjustments should be made based
   * off this datasheet:
   * https://cdn.sos.sk/productdata/a8/47/608a6927/sen54-sdn-t.pdf
   *
  ...
  uint16_t setVocAlgorithmTuningParameters(int16_t indexOffset,
                                           ...
                                           int16_t stdInitial,
                                           int16_t gainFactor) {
    return sen5x.setVocAlgorithmTuningParameters(
        indexOffset, learningTimeOffsetHours, learningTimeGainHours,
  ...
  uint16_t setNoxAlgorithmTuningParameters(int16_t indexOffset,
                                           ...
                                           int16_t stdInitial,
                                           int16_t gainFactor) {
    return sen5x.setNoxAlgorithmTuningParameters(
        indexOffset, learningTimeOffsetHours, learningTimeGainHours,
        gatingMaxDurationMinutes, stdInitial, gainFactor);
  };
  /**
  ...
  /**
   * Reset relevant values to 0, this is useful for preparing for another
   * measurement cycle.
...
private:
  ...
  bool readNumVals; // Do we want to read the number concentration and typical
                    // particle size?
  ...
  int pmReadFrequency =
      0; // Counter for the number of times we have read the PM values
};