 ...
 *
 * Product Page:
 * https://www.co2meter.com/products/k-30-co2-sensor-module?variant=8463942
...
public:
   ...
   * @param warmUp If we should wait 6 minutes before collecting data to allow
   * the sensor to warm up
   * @param valMult How much to multiply the recorded output by
   */
  Loom_K30(Manager &man, bool useMux = false, int addr = 0x68,
           bool warmUp = true, int valMult = 1);
...
private:
  ...
  byte read_CO2[7] = {
      0xFE, 0X44, 0X00, 0X08,
      0X02, 0X9F, 0X25}; // Command packet to read Co2 (see app note)
  byte sensor_response[7] = {0, 0, 0, 0,
                             0, 0, 0}; // create an array to store the response
