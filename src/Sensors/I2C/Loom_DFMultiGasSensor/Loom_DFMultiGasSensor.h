...
public:
   ...
   * @param initializationRetyLimit How many attempts will be made to initialize
   * the sensor before deeming it a failure and moving on
   * @param sensorPowersDown Does the sensor power down during sleep and need to
   * re connect each power_up
  ...
  Loom_DFMultiGasSensor(Manager &man, uint8_t address = 0x74,
                        ...
                        bool sensorPowersDown = false, bool useMux = false);
