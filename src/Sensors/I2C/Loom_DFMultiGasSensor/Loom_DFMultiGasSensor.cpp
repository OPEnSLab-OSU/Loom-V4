...
Loom_DFMultiGasSensor::Loom_DFMultiGasSensor(Manager &man, uint8_t address,
                                             ...
                                             bool sensorPowersDown, bool useMux)
    : I2CDevice("DFR_MultiGasSensor"), manInst(&man),
      retryLimit(initializationRetyLimit), gasSensor(&Wire, address) {
...
void Loom_DFMultiGasSensor::power_up() {
  ...
  if (powersDown) {
    // If we are unable to connect to the sensor in power up this should disable
    // the module for atleast this run, decide if we want to do this or not
    moduleInitialized = attemptConnectionToSensor();
  } else {
    // Module assumed to be initialized and to have called .begin() if never
    // powered down Prevents too many duplicate .begin() calls moduleInitialized
  ...
  if (moduleInitialized) {
    ...
    ERROR(F(
        "DFRobot Multi Gas sensor failed to power on and has been disabled."));
  }
...
bool Loom_DFMultiGasSensor::attemptConnectionToSensor() {
  ...
  for (uint8_t retryCount = 0; retryCount < retryLimit; retryCount++) {
    ...
    // If we do successfully begin the sensor we want to stop the loop
    // immediatly and move on to the next part of initialization
    if (gasSensor.begin()) {
    ...
    // If we have reached the max number of retries, then we want to just stop
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_DFMultiGasSensor::configureSensorProperties(
    DFRobot_GAS::eMethod_t aquireMode, DFRobot_GAS::eSwitch_t gasCompMode) {
  ...
  delay(1000);
  LOGF("Acquire Mode set to %hs",
       aquireMode == gasSensor.PASSIVITY ? "PASSIVE" : "INITIATIVE");

  // Set temperature compensation
  LOG(F("Setting temp compensation..."));
  gasSensor.setTempCompensation(gasSensor.ON);
  delay(1000);
  LOGF("Temp compensation set to %hs",
       gasCompMode == gasSensor.OFF ? "OFF" : "ON");
}
//////////////////////////////////////////////////////////////////////////////////////////////////////
