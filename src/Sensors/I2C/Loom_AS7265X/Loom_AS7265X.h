...
public:
   ...
   * @param mode Read Mode: 0("4 channels out of 6"), 1("Different 4 channels
   * out of 6"), 2("All 6 channels continuously"), 3("One-shot reading of all
   * channels")
   * @param integration_time Integration time (time will be 2.8ms * [integration
   * value])
   */
  Loom_AS7265X(Manager &man, bool useMux = false, int addr = 0x49,
               bool use_bulb = false, uint8_t gain = 64,
...
private:
  ...
  uint16_t
      nir[6]; // Measured near-infra-red bands values. Units: counts / (μW/cm^2)
