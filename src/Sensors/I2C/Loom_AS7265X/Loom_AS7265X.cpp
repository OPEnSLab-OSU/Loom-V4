...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_AS7265X::Loom_AS7265X(Manager &man, bool useMux, int addr, bool use_bulb,
                           uint8_t gain, uint8_t mode, uint8_t integration_time)
    : I2CDevice("AS7265X"), manInst(&man), use_bulb(use_bulb), gain(gain),
      mode(mode), integration_time(integration_time) {
  module_address = addr;
...
void Loom_AS7265X::initialize() {
  if (asInst.begin() == false) {
    ERROR(
        F("Failed to initialize AS7265X! Check connections and try again..."));
