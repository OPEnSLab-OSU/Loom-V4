...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_ZXGesture::Loom_ZXGesture(Manager &man, int address, bool useMux,
                               Mode mode)
    : I2CDevice("ZX Gesture"), manInst(&man), zx(ZX_Sensor(address)),
      mode(mode) {
...
void Loom_ZXGesture::initialize() {
  ...
  if (!zx.init()) {
    ERROR(F("Failed to initialize ZX Gesture Sensor! Check connections and try "
            "again..."));
  } else {
    ...
    if (ver != ZX_REG_MAP_VER) {
      ...
      snprintf(output, OUTPUT_SIZE,
               "Incorrect Register Map Version. Expected Version: %u Actual "
               "Version: %u",
               ZX_REG_MAP_VER, ver);
      ERROR(output);
      return;
