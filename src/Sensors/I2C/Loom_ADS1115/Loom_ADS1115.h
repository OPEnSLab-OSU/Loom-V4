...
public:
   ...
   *  @param enable_diff If we want to read the differential data from the
   * sensor
   *  @param gain How much gain to apply to the readings.
   */
  Loom_ADS1115(Manager &man, byte address = ADS1X15_ADDRESS,
               bool useMux = false, bool enable_analog = true,
               bool enable_diff = false, adsGain_t gain = adsGain_t::GAIN_ONE);
