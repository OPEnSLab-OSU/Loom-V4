...
private:
  ...
  void
  selectPin(uint8_t pin); // Select which pin of the multiplexer to transmit to
  void disableChannels(); // Disables all channels on the Multiplexer
  bool isDeviceConnected(
      byte addr); // Check if there is a device at the specified address

  void refreshSensors(); // Checks to see if any new sensors were swapped in
                         // allows for hot swapping
  Module *loadSensor(
      const byte addr); // Load the correct sensor based on the I2C address
   ...
   */
  const std::array<byte, 9> alt_addresses = {0x71, 0x72, 0x73,
                                             0x74, 0x75, 0x78};
};