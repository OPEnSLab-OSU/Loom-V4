...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_Multiplexer::Loom_Multiplexer(Manager &man,
                                   const std::vector<byte> &addresses)
    : Module("Multiplexer"), manInst(&man) {
  moduleInitialized = false;
  manInst->registerModule(this);
...
void Loom_Multiplexer::initialize() {
          ...
          if (addr > 0) {
            ...
            if (isDeviceConnected(addr)) {
              snprintf(output, OUTPUT_SIZE,
                       "Found I2C Device on Pin %i at address %x", i, addr);
              LOG(output);

              sensors.push_back(std::make_tuple(addr, loadSensor(addr), i));

              // Initialize connected sensor
              snprintf(output, OUTPUT_SIZE, "%s_%i",
                       std::get<1>(sensors[moduleIndex])->getModuleName(), i);
  ...
  ERROR(F(
      "Multiplexer was not found at the standard address or any alternatives"));
  FUNCTION_END;
}
//////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (byte addr : known_addresses) {
      ...
      if (isDeviceConnected(addr) && addr > 0) {
        ...
        if (std::get<1>(sensors[moduleIndex])->module_address != addr) {
          ...
          snprintf(
              output, OUTPUT_SIZE,
              "New sensor detected on port %i at I2C address %x of type %s", i,
              addr, std::get<1>(sensors[moduleIndex])->getModuleName());
          LOG(output);
