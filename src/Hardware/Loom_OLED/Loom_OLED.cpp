...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_OLED::Loom_OLED(Manager &man, const bool enable_rate_filter,
                     const uint16_t min_filter_delay, const Version type,
                     const byte reset_pin, const Format display_format,
                     const uint16_t scroll_duration, const byte freeze_pin,
                     const FreezeType freeze_behavior)
    : Module("OLED"), manInst(&man), min_filter_delay(min_filter_delay),
      version(type), reset_pin(reset_pin), display_format(display_format),
      scroll_duration(scroll_duration), freeze_behavior(freeze_behavior),
      freeze_pin(freeze_pin), flattenedDoc(MAX_JSON_SIZE) {
  manInst->registerModule(this);

  // Create the correct display module given the OLED version
  display = (version == Version::FEATHERWING) ? new Adafruit_SSD1306()
                                              : new Adafruit_SSD1306(reset_pin);
}
//////////////////////////////////////////////////////////////////////////////////////////////////////

//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_OLED::~Loom_OLED() { delete display; }
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Loom_OLED::initialize() {
  ...
  // Start the I2C device on 0x3C for the 128x32 display, address cannot be
  // changed
...
void Loom_OLED::display_data() {
  ...
  // If we are trying to write data check if we are freezing the display if so
  // return
  ...
  case Format::SCROLL:
    ...
    int offset =
        size * (float(time % (scroll_duration)) / (float)(scroll_duration));
...
bool Loom_OLED::canWrite() {
  if ((millis() > min_filter_delay) &&
      ((millis() - lastLogTime) < min_filter_delay)) {
...
void Loom_OLED::flattenJSONObject(JsonObject json) {
  ...
  // Using our temp doc we convert it to and object and create a new nested
  // object that is called flatObj
  JsonObject flatData =
      flattenedDoc.to<JsonObject>().createNestedObject("flatObj");
