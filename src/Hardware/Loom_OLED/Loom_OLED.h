...
class Loom_OLED : public Module {
  ...
  public:
     ...
     *
     * @param enable_rate_filter		Whether or not to impose maximum update
     * rate
     * @param min_filter_delay		    Minimum update delay, if
     * enable_rate_filter enabled
     * @param type					    Which version of the
     * OLED is being used (Version::FEATHERWING or Version::BREAKOUT)
     * @param reset_pin				    Which pin should be used for
     * reseting. Only applies to breakout version
     * @param display_format			How to display the key value
     * pairs of a bundle (Format::Scroll, Format::FOUR or Format::EIGHT)
     * @param scroll_duration			The time (ms) to complete full
     * scroll cycle if display_format is SCROLL
     * @param freeze_pin				Which pin should be used to
     * pause the display
     * @param freeze_behavior			How freezing the display should
     * behave (FreezeType::SCROLL, FreezeType::DATA or FreezeType::DISABLE)
     */
    Loom_OLED(Manager &man, const bool enable_rate_filter = true,
              const uint16_t min_filter_delay = 300, const Version type = Version::FEATHERWING,
              const byte reset_pin = A2, const Format display_format = Format::SCROLL,
              const uint16_t scroll_duration = 6000, const byte freeze_pin = 10,
  ...
  private:
    ...
    uint scroll_duration;       // The duration to complete a full cycle through a
                                // bundle of data (milliseconds)(non-blocking)
