...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_TippingBucket::Loom_TippingBucket(Manager &man, COUNTER_TYPE type,
                                       float inchesPerTip)
...
void Loom_TippingBucket::measure() {
  /* First check if we are actually meant to be reading the values from our I2C
   * device, this shouldn't occur if we are using interrupts */
  ...
  /* If we are using the hypnos we want to check the RTC to calculate how many
   * tips occurred in the last hour*/
  if (hypnosInst != nullptr) {
    if (times.size() <= 1) {
      times.push_front(
          hypnosInst->getCurrentTime()
              .unixtime()); // Push the start time onto the front of the queue
      tips.push_front(tipCount -
                      lastTipCount); // Push the current number of tips onto the
                                     // front of the queue
    } else {
      // If it has been more than one hour since the oldest tip, we want to
    ...
    // Set the hourly tips back to zero so we can re-calculate it with the new
    // data
    hourlyTips = 0;

    /* Loop over the last hour to accumlate the number of tips that occured
     * within the last hour and we want to subtract the current value minus the
     * last to get the difference and add that*/
