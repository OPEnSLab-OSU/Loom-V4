...
bool SDManager::writeLineToFile(const char *filename, const char *content) {
  ...
  if (sdInitialized) {
    ...
    // Check if the file was actually opened, if so write the content to the
    // file
    if (myFile) {
  ...
  printModuleName("SD Card was improperly initialized and as such this "
...
void SDManager::writeHeaders() {
  ...
  // Append the serial number to the top of the CSV file, reset the header1
  // array
  ...
  // If there is a key that contains timestamp data when need to include that
  // separately
...
bool SDManager::log(DateTime currentTime) {
  ...
  if (sdInitialized) {
    ...
    // Open the file in read/write mode, create the file if we need to and
    // append the content to the end of the file
    myFile = sd.open(fileName, O_RDWR | O_CREAT | O_APPEND);

    if (myFile) {

      // If this file has never been written to before we need to create and
      // write the proper headers to the file
      ...
      snprintf_P(output, MAX_JSON_SIZE, PSTR("%s,%i,"),
      ...
      // If there is a key that contains timestamp data when need to include
      // that separately
      if (document.containsKey("timestamp")) {
        char utcArr[21];
        char localArr[21];
        ...
        strncpy(utcArr, document["timestamp"]["time_utc"].as<const char *>(),
                21);
        strncpy(localArr,
                document["timestamp"]["time_local"].as<const char *>(), 21);
      ...
      snprintf_P(output, MAX_JSON_SIZE, PSTR("Successfully logged data to %s"),
                 fileName);
      LOG(output);
...
bool SDManager::begin() {
  ...
  if (!sd.begin(chip_select, SD_SCK_MHZ(50))) {
    printModuleName(
        "Failed to Initialize SD Card! SD Card functionality will be disabled, "
  ...
  if (!sdInitialized) {
    // Try to open the root of the file system so we can get the files on the
    // device
  ...
  // Once the SD card has initialized the first round through we don't want to
...
bool SDManager::updateCurrentFileName() {
  ...
  if (strlen(overrideFileName) > 0) {
    ...
    snprintf_P(fileName, 260, PSTR("%s%i.csv"), overrideFileName,
    ...
    snprintf_P(fileName, 260, PSTR("%s%i.csv"), device_name,
               getCurrentFileNumber());
    snprintf_P(fileNameNoExtension, 260, PSTR("%s%i"), device_name,
               getCurrentFileNumber());
    snprintf_P(batchFileName, 260, PSTR("%s-Batch.txt"), fileNameNoExtension);
  }
...
void SDManager::logBatch() {
  ...
  // Check if the file has been opened properly and write the JSON packet to one
  // line
  if (myFile) {
