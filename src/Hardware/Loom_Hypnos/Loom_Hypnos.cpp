...
volatile bool Loom_Hypnos::shouldPowerUp;
...
void Loom_Hypnos::sleep(uint32_t seconds, bool waitForSerial) {
    ...
    shouldPowerUp = false;
    LowPower.attachInterruptWakeup(RTC_ALARM_WAKEUP, Loom_Hypnos::wakeup, 0);
    LowPower.sleep(seconds * 1000);  // Go to sleep and hang

    // Resume sleep if awoken prematurely by different interrupt
