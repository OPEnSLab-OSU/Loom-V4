...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_Hypnos::Loom_Hypnos(Manager &man, HYPNOS_VERSION version, TIME_ZONE zone,
                         bool use_custom_time, bool useSD)
    : Module("Hypnos"), custom_time(use_custom_time), sd_chip_select(version),
      enableSD(useSD), timezone(zone) {
  manInst = &man;
...
bool Loom_Hypnos::registerInterrupt(InterruptCallbackFunction isrFunc,
                                    int interruptPin,
                                    HypnosInterruptType interruptType,
                                    int triggerState) {
  FUNCTION_START;
  pinMode(interruptPin, INPUT_PULLUP); //  Set interrupt pin input mode
  LOG(F("Registering interrupt..."));

  // If the RTC hasn't already been initialized then do so now if we are trying
  // to schedule an RTC interrupt
  ...
  if (isrFunc != nullptr) {
    ...
    // If the interrupt we registered is for sleep we should set the interrupt
    ...
    if (interruptType == SLEEP) {
      ...
      attachInterrupt(digitalPinToInterrupt(interruptPin), isrFunc,
                      triggerState);
      attachInterrupt(digitalPinToInterrupt(interruptPin), isrFunc,
                      triggerState);
      LOG(F("Interrupt successfully attached!"));
    ...
    pinToInterrupt.insert(std::make_pair(
        interruptPin, std::make_tuple(isrFunc, triggerState, interruptType)));
    FUNCTION_END;
    return true;
  } else {
    detachInterrupt(digitalPinToInterrupt(interruptPin));
    ERROR(
        F("Failed to attach interrupt! Interrupt callback evaluated to a null "
          "pointer, it is possible you forgot to supply a callback function"));
    FUNCTION_END;
    return false;
  }
  FUNCTION_END;
...
bool Loom_Hypnos::reattachRTCInterrupt(int interruptPin) {
  ...
  if (std::get<2>(pinToInterrupt[interruptPin]) != SLEEP) {
    ...
    // If we haven't previously registered the interrupt we need to do this
    // before we can reattach to an interrupt that doesn't exist
    if (pinToInterrupt.count(interruptPin) <= 0) {
      ERROR(F("Failed to reattach interrupt! Interrupt has not previously been "
              "registered..."));
      FUNCTION_END;
      return false;
    }
    ...
    LowPower.attachInterruptWakeup(interruptPin,
                                   std::get<0>(pinToInterrupt[interruptPin]),
                                   std::get<1>(pinToInterrupt[interruptPin]));
  }
...
void Loom_Hypnos::wakeup() {
  detachInterrupt(
      pinToInterrupt.begin()
          ->first); // Detach the interrupt so it doesn't trigger again
}
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Loom_Hypnos::initializeRTC() {
  ...
  if (!RTC_DS.begin()) {
    ERROR(F("Couldn't start RTC! Check your connections... Execution will now "
            "hang as this is likely a fatal error"));
    return;
  }

  // This may end up causing a problem in practice - what if RTC loses power in
  // field? Shouldn't happen with coin cell batt backup
  if (RTC_DS.lostPower()) {
  ...
  snprintf(output, OUTPUT_SIZE, "Custom time successfully set to: %s",
           getCurrentTime().text());
  LOG(output);
  FUNCTION_END;
}
...
bool Loom_Hypnos::isDaylightSavings() {
  ...
  if (timezone == AST || timezone == EST || timezone == CST ||
      timezone == AST || timezone == PST || timezone == AKST) {
...
DateTime Loom_Hypnos::getCurrentTime() {
  if (RTC_initialized)
    ...
    LOG(F("Attempted to pull time when RTC was not previously initialized! "
...
bool Loom_Hypnos::networkTimeUpdate() {
  ...
  if (networkComponent != nullptr && networkComponent->isConnected()) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (networkComponent->getNetworkTime(&year, &month, &day, &hour, &minute,
                                           &second, &tz)) {
    ...
    ERROR("Network component not set in hypnos or component wasn't connected "
          "to the internet.");
  }
  FUNCTION_END;
}
...
void Loom_Hypnos::dateTime_toString(DateTime time, char array[21],
  ...
  if (isLocal) {
    snprintf_P(array, 21, PSTR("%u-%02u-%02uT%u:%u:%u"), time.year(),
               time.month(), time.day(), time.hour(), time.minute(),
               time.second());
  } else {
    snprintf_P(array, 21, PSTR("%u-%02u-%02uT%u:%u:%uZ"), time.year(),
               time.month(), time.day(), time.hour(), time.minute(),
               time.second());
  }
}
...
void Loom_Hypnos::set_custom_time() {
  ...
  RTC_DS.adjust(DateTime(computer_year.toInt(), computer_month.toInt(),
                         computer_day.toInt(), computer_hour.toInt(),
                         computer_min.toInt(), computer_sec.toInt()));
  RTC_initialized = true;
...
void Loom_Hypnos::sleep(bool waitForSerial) {
  ...
  // If the alarm set time is less than the current time we missed our next
  // alarm so we need to set a new one, we need to check if we have powered on
  // already so we dont use the RTC that isn't enabled
  ...
  if (shouldPowerUp) {
    ...
    // After powering down the devices check if the alarmed time is less than
    // the current time, this means that the alarm may have already triggered
  ...
  // If it has we want to trigger a resample which requires powering the sensors
  // back up
  else {
    WARNING("Alarm triggered during sample, specified sample duration was too "
            "short! Resampling...");
...
void Loom_Hypnos::post_sleep(bool waitForSerial) {
  ...
  if (shouldPowerUp) {
    ...
    enable(
        enable33,
        enable5); // Checks if the 3.3v or 5v are disabled and re-enables them
    Watchdog.reset();
    delay(1000);
    Watchdog.reset();
    ...
    // We want to wait for the user to re-open the serial monitor before
    // continuing to see readouts
    if (waitForSerial) {
      TIMER_DISABLE;
      while (!Serial)
        ;
...
TimeSpan Loom_Hypnos::getConfigFromSD(const char *fileName) {
  ...
  DeserializationError deserialError =
      deserializeJson(doc, (const char *)fileRead);
  free(fileRead);
  ...
  if (deserialError != DeserializationError::Ok) {
    snprintf(output, OUTPUT_SIZE,
             "There was an error reading the config from SD: %s, defaulting "
    ...
    if (!json["timezone"].isNull()) {
      ...
      snprintf(output, OUTPUT_SIZE, "Selected timezone: %s, UTC offset: %i",
               timezoneStr, (int)timezoneMap[(const char *)timezoneStr]);
      LOG(output);
    ...
    if (!json["SleepInterval"].isNull()) {
      ...
      snprintf(output, OUTPUT_SIZE,
               "There was an error reading the sampling interval from SD, "
