...
/**
 * Class used to manage interaction with the SD card read/writer on the Hypnos
 * board
 *
...
public:
   ...
   * @param currentTime The current time provided by the RTC this allows us to
   * set accurate modified/created times for files
   */
  bool log(DateTime currentTime);

  /**
   * Read the contents of a given file on the SD card and return them as a
   * string
   *
...
private:
  ...
  char device_name[100]; // Device name of the whole thing used as the starting
                         // point of the SD file name

  char batchFileName[260]; // File name to log batches to
  char fileName[260];      // Current file name that data is being logged to
  char fileNameNoExtension[260]; // Current file name that data is being logged
                                 // to without the file extension
  ...
  char *headers[2]; // Contains the main and sub headers that are added to the
                    // top of the CSV files

  void logBatch(); // Log data in batch format

  void writeHeaders(); // Create the headers for the CSV file based off what
                       // info we are storing
  bool updateCurrentFileName(); // Update the current file name to log to based
                                // on files already existing on the SD card
};