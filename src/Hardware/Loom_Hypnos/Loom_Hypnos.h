...
// Custom comparator for const char*, used for evaluating timezone name to
// timezone enum
struct cmp_str {
...
/**
 * All in one driver for the Hypnos board. This allows users to use the Hypnos
 * board in a more modularized manner not requiring all the Loom stuff.
 *
...
public:
  ...
  /**
   * Constructs a new Hypnos Instance using the manager to hold information
   * about the device
   * @param man Reference to the manager
   * @param version The version of the Hypnos in use, this changes which pin is
   * used as and SD chip select
   * @param timezone The current timezone the clock was set to
   * @param use_custom_time Use a specific time set by the user that is
   * different than the compile time
   * @param useSD Whether or not SD card functionality should be enabled
   */
  Loom_Hypnos(Manager &man, HYPNOS_VERSION version, TIME_ZONE zone,
              bool use_custom_time = true, bool useSD = true);
   ...
   * Disables the Power Rails and sets the SPI pins to INPUT which effectively
   * disables them
   */
   ...
   */
  void setSleepConfiguration(POWERRAIL_CONFIG config) {
    sleepModePowerConfig = config;
  };
   ...
   * @param interruptPin Defaults to RTC pin on Hypnos can be changed to reflect
   * other interrupts
  ...
  /**
   * Called when the user wants to reattach the interrupt handler to the RTC
   * interrupt to collect subsequent interrupts
   * @param interruptPin Pin to reattach the interrupt to for RTC this doesn't
   * need to be changed
   */
   ...
   * @param duration The time that will elapse before the next interrupt is
   * triggered
   */
  void setInterruptDuration(const TimeSpan duration);

  /**
   * Drops the Feather M0 and Hypnos board into a low power sleep waiting for an
   * interrupt to wake it up and pull it out of sleep
   * @param waitForSerial Whether or not we should wait for the user to open the
   * serial monitor before continuing execution
   */
  ...
  /**
   * Load the configuration for the hypnos from the SD card (Timezeone and sleep
   * interval)
   * @param fileName The file name on the root of the SD card to retrieve the
   * information from
  ...
  void setNetworkInterface(NetworkComponent *component) {
...
private:
  ...
  NetworkComponent *networkComponent =
      nullptr; // Reference to a NetworkComponent
  ...
  /**
   * Based on the state we are entering determine the configuration of the 3V
   * power rail
  ...
  /**
   * Based on the state we are entering determine the configuration of the 5V
   * power rail
   * @param state The new state teh device is entering
   */
  ...
  int sd_chip_select; // Pin that the SD card will use to communicate with the
                      // Hypnos
  bool enableSD; // Specifies whether or not the SD card should be enabled on
                 // the Hypnos
  ...
  std::map<int, std::tuple<InterruptCallbackFunction, int, HypnosInterruptType>>
      pinToInterrupt;
  ...
  std::map<const char *, TIME_ZONE, cmp_str>
      timezoneMap; // String to Timezone enum, use custom compare to ensure that
                   // strings are compared correctly

  DateTime
  getLocalTime(DateTime time); // Convert a given UTC time to local time
  ...
  /* Sleep functionality */
  void pre_sleep(); // Called just before the hypnos enters sleep, this
                    // disconnects the power rails and the serial bus
  void post_sleep(
      bool waitForSerial); // Called just after the hypnos wakes up, this
                           // reconnects the power rails and the serial bus
};
