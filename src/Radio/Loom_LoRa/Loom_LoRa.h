...
public:
   ...
   * @param sendMaxRetries The number of transmission attempts to make before
   * failing
   * @param receiveMaxRetries The number of reception attempts to make before
   * failing
   ...
   * @param retryTimeout Length of time between retransmissions (ms)
   */
  Loom_LoRa(Manager &manager, const uint8_t powerLevel = 23,
...
private:
  ...
  std::unordered_map<uint8_t, PartialPacket>
      frags; // Partial packets sorted by address
