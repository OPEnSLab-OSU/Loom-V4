...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_LoRa::Loom_LoRa(Manager &manager, const uint8_t address,
                     const uint8_t powerLevel, const uint8_t sendMaxRetries,
                     const uint8_t receiveMaxRetries,
                     const uint16_t retryTimeout)
    : Module("LoRa"), manager(&manager), radioDriver{RFM95_CS, RFM95_INT},
      deviceAddress(address), powerLevel(powerLevel),
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_LoRa::Loom_LoRa(Manager &manager, const uint8_t powerLevel,
                     const uint8_t retryCount, const uint16_t retryTimeout)
    : Loom_LoRa(manager, manager.get_instance_num(), powerLevel, retryCount,
...
bool Loom_LoRa::receiveFromLoRa(uint8_t *buf, uint8_t buf_size, uint timeout,
  ...
  if (timeout) {
    status =
        radioManager->recvfromAckTimeout(buf, &buf_size, timeout, fromAddress);
  } else {
...
bool Loom_LoRa::sendFragmentedPacket(JsonObject json,
  ...
  LOG(F("Packet was greater than the maximum packet length; the packet will be "
        "fragmented"));
  bool status = false;
  ...
  if (!status) {
    ERROR(F("Unable to transmit initial packet header! Split packets will not "
            "be sent"));
    return false;
  }
