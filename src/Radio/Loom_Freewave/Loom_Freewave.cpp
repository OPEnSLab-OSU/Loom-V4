...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_Freewave::Loom_Freewave(Manager &man, const uint8_t address,
...
bool Loom_Freewave::receive(uint maxWaitTime) {
  ...
  if (maxWaitTime == 0) {
    ...
    recvStatus =
        manager->recvfromAckTimeout(buffer, &len, maxWaitTime, &fromAddress);
  }
  ...
  if (recvStatus) {
    ...
    // Update device name
    manInst->set_device_name(
        manInst->getDocument()["id"]["name"].as<const char *>());
    manInst->set_instance_num(
        manInst->getDocument()["id"]["instance"].as<int>());
...
bool Loom_Freewave::send(const uint8_t destinationAddress) {
  ...
  if (!manager->sendtoWait((uint8_t *)buffer, sizeof(buffer),
                           destinationAddress)) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_Freewave::power_up() { driver.available(); }
//////////////////////////////////////////////////////////////////////////////////////////////////////
