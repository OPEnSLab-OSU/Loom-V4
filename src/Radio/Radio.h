...
/**
 * Slightly abstracted Radio class to allow for all radio modules to inherit
 * from one spot
 *
 * WARNING: this class was previously used by both LoRa and Freewave, but after
 * refactoring is now only used by Freewave. At some point this class should
 * probably be merged into Freewave and removed.
 *
...
protected:
  ...
  StaticJsonDocument<1000>
      recvDoc; // Individual Document Representing what is being received
  StaticJsonDocument<1000>
      sendDoc; // Individual Document Representing what is being sent
   ...
   * expires or a packet is received
  ...
  bool bufferToJson(uint8_t *buffer) {
    ...
    if (error != DeserializationError::Ok) {
      snprintf(output, OUTPUT_SIZE, "Error occurred parsing MsgPack: %s",
               error.c_str());
      ERROR(output);
      return false;
    }
