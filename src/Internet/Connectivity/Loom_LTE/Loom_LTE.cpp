...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_LTE::Loom_LTE(Manager &man, const char *apn, const char *user,
                   const char *pass, const int pin, LTE_VERSION version)
...
void Loom_LTE::powerBoardOff() {
  // NOTE: We don't need to power off the sparkfun LTE board we can just use the
  // power off command Handle powering off the parkfun board
...
void Loom_LTE::initialize() {
  ...
  if (modemInfo == NULL) {
    ERROR(F("LTE shield not detected! This can also be triggered if there "
            "isn't a SIM card in the board"));
    moduleInitialized = false;
    FUNCTION_END;
    return;
  } else {
...
void Loom_LTE::power_up() {
  FUNCTION_START;
  // If the batch_sd is initialized and the current batch is one less than the
  // maximum so we turn on the device before the last batch
...
void Loom_LTE::power_down() {
  ...
  if (moduleInitialized && powerUp) {
    ...
    modem.poweroff();
    // // We must pull the power pin low for 3.5 seconds to trigger a power on,
    // and then release the pin state pull();
    powered = false;
...
bool Loom_LTE::connect() {
  ...
  do {
    ...
    snprintf(output, OUTPUT_SIZE, "Attempting to connect to LTE Network: %s",
             APN);
    LOG(output);
    if (modem.gprsConnect(APN, gprsUser, gprsPass)) {
      ...
      snprintf(output, OUTPUT_SIZE, "Connection failed %u / 10. Retrying...",
               attemptCount);
      WARNING(output);
      delay(10000);
      attemptCount++;
...
bool Loom_LTE::verifyConnection() {
  ...
  if (!client.connect("vsh.pp.ua", 80)) {
    ERROR(F("Failed to contact TinyGSM example your internet connection may "
            "not be completely established!"));
    client.stop();
    FUNCTION_END;
    return false;
  } else {
...
void Loom_LTE::loadConfigFromJSON(char *json) {
  ...
  if (deserialError != DeserializationError::Ok) {
    snprintf(output, OUTPUT_SIZE,
             "There was an error reading the WIFI credentials from SD: %s",
             deserialError.c_str());
    ERROR(output);
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
bool Loom_LTE::getNetworkTime(int *year, int *month, int *day, int *hour,
                              int *minute, int *second, float *tz) {
