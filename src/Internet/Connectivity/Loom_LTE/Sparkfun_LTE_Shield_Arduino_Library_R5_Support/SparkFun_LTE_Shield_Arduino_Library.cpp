/*
  Arduino Library for the SparkFun LTE CAT M1/NB-IoT Shield - SARA-R4 / SARA-R5 test build
...
#define LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT 2000
...
const char LTE_SHIELD_REGISTRATION_STATUS_CREG[] = "+CREG";
const char LTE_SHIELD_REGISTRATION_STATUS_CEREG[] = "+CEREG";
...
const unsigned long LTE_SHIELD_SUPPORTED_BAUD[NUM_SUPPORTED_BAUD] =
    {
        ...
        57600,
        230400};
#ifndef LTE_SHIELD_DEFAULT_BAUD_RATE
#define LTE_SHIELD_DEFAULT_BAUD_RATE 9600
#endif

char lteShieldRXBuffer[192];
...
LTE_Shield::LTE_Shield(uint8_t powerPin, uint8_t resetPin)
{
    ...
    memset(lteShieldRXBuffer, 0, sizeof(lteShieldRXBuffer));
}
...
boolean LTE_Shield::begin(HardwareSerial &hardSerial, unsigned long baud, boolean powerCycleBeforeAT)
{
    LTE_Shield_error_t err;

    _hardSerial = &hardSerial;

    if (powerCycleBeforeAT)
    {
        powerOn();
        delay(10000);
    }
...
LTE_Shield_error_t LTE_Shield::sendRawCommand(const char *command, char *responseDest, size_t responseSize, unsigned long commandTimeout, boolean prependAT)
{
    if (responseDest != NULL && responseSize > 0)
    {
        responseDest[0] = '\0';
    }
    ...
    if (responseDest == NULL || responseSize == 0)
    {
        return sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, NULL, commandTimeout, prependAT);
    }

    LTE_Shield_error_t err = sendCommandWithResponse(command, LTE_SHIELD_RESPONSE_OK, responseDest, commandTimeout, prependAT);
    responseDest[responseSize - 1] = '\0';
    return err;
}

void LTE_Shield::printRawCommand(const char *command, unsigned long commandTimeout, boolean prependAT)
{
    char response[192];
    memset(response, 0, sizeof(response));

    Serial.print(F("LTE raw "));
    if (prependAT)
    {
        Serial.print(F("AT"));
    }
    ...
    LTE_Shield_error_t err = sendRawCommand(command, response, sizeof(response), commandTimeout, prependAT);
    ...
    Serial.print(F(" ["));
    Serial.print(errorString(err));
    Serial.println(F("]"));
}

const char *LTE_Shield::errorString(LTE_Shield_error_t error)
{
    switch (error)
    {
    case LTE_SHIELD_ERROR_SUCCESS:
        return "SUCCESS";
    case LTE_SHIELD_ERROR_OUT_OF_MEMORY:
        return "OUT_OF_MEMORY";
    case LTE_SHIELD_ERROR_TIMEOUT:
        return "TIMEOUT";
    case LTE_SHIELD_ERROR_UNEXPECTED_PARAM:
        return "UNEXPECTED_PARAM";
    case LTE_SHIELD_ERROR_UNEXPECTED_RESPONSE:
        return "UNEXPECTED_RESPONSE";
    case LTE_SHIELD_ERROR_NO_RESPONSE:
        return "NO_RESPONSE";
    case LTE_SHIELD_ERROR_DEREGISTERED:
        return "DEREGISTERED";
    case LTE_SHIELD_ERROR_INVALID:
    default:
        return "INVALID";
    }
}
...
LTE_Shield_registration_status_t LTE_Shield::registration(void)
{
    ...
    const char *registrationCommand = _saraR5Enabled ? LTE_SHIELD_REGISTRATION_STATUS_CEREG : LTE_SHIELD_REGISTRATION_STATUS_CREG;

    command = lte_calloc_char(strlen(registrationCommand) + 2);
    if (command == NULL)
        return LTE_SHIELD_REGISTRATION_INVALID;
    sprintf(command, "%s?", registrationCommand);
...
boolean LTE_Shield::setNetwork(mobile_network_operator_t mno)
{
    ...
    if (_saraR5Enabled)
    {
        return setMno(mno) == LTE_SHIELD_ERROR_SUCCESS;
    }
...
LTE_Shield_error_t LTE_Shield::getAPN(String *apn, IPAddress *ip, uint8_t cid)
{
    LTE_Shield_error_t err;
    char *command;
    char *response;
...
LTE_Shield_error_t LTE_Shield::registerOperator(struct operator_stats oper)
{
    ...
    command = lte_calloc_char(strlen(LTE_SHIELD_OPERATOR_SELECTION) + 32);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
    if (_saraR5Enabled)
    {
        sprintf(command, "%s=1,2,\"%lu\",%u", LTE_SHIELD_OPERATOR_SELECTION, oper.numOp, oper.act);
    }
    else
    {
                                    ...
                                    LTE_Shield::LTE_Shield_init_type_t initType)
{
    ...
    if (_saraR5Enabled)
    {
        ...
        sendCommandWithResponse("+CMEE=2", LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT);
        sendCommandWithResponse("+CEREG=2", LTE_SHIELD_RESPONSE_OK, NULL, LTE_SHIELD_STANDARD_RESPONSE_TIMEOUT);
...
LTE_Shield_error_t LTE_Shield::setMno(mobile_network_operator_t mno)
{
    ...
    command = lte_calloc_char(strlen(LTE_SHIELD_COMMAND_MNO) + 6);
    if (command == NULL)
        return LTE_SHIELD_ERROR_OUT_OF_MEMORY;
