/*
  ...
  SARA-R4 and SARA-R5 modules over either a SoftwareSerial or hardware serial port.
...
public:
    ...
    // Select SARA-R5 behavior at runtime. Default is false so old SARA-R4 sketches keep their original behavior.
    void setSaraR5Mode(boolean enabled = true) { _saraR5Enabled = enabled; };
    boolean isSaraR5Mode(void) const { return _saraR5Enabled; };

    // Begin -- initialize BT module and ensure it's connected
#ifdef LTE_SHIELD_SOFTWARE_SERIAL_ENABLED
    boolean begin(SoftwareSerial &softSerial, unsigned long baud = 9600);
#endif
    boolean begin(HardwareSerial &hardSerial, unsigned long baud = LTE_SHIELD_DEFAULT_BAUD_RATE);
    boolean begin(HardwareSerial &hardSerial, unsigned long baud, boolean powerCycleBeforeAT);
    ...
    LTE_Shield_error_t sendRawCommand(const char *command, char *responseDest, size_t responseSize, unsigned long commandTimeout = 2000, boolean prependAT = true);
    void printRawCommand(const char *command, unsigned long commandTimeout = 2000, boolean prependAT = true);
    const char *errorString(LTE_Shield_error_t error);
    ...
    LTE_Shield_error_t getAPN(String *apn, IPAddress *ip, uint8_t cid = 1);
