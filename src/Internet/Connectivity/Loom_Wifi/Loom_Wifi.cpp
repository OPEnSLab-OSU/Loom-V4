...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_WIFI::Loom_WIFI(Manager &man, CommunicationMode mode, const char *name,
                     const char *password, int connectionRetries)
    : NetworkComponent("WiFi"), manInst(&man), mode(mode),
...
void Loom_WIFI::power_up() {
  // If batchSD is defined and our current batch is not equal to one less than
  // the needed for publishing dont power up
...
void Loom_WIFI::connect_to_network() {
  ...
  if (strlen(wifi_password) > 0) {
    ...
    while (WiFi.begin(wifi_name, wifi_password) != WL_CONNECTED) {
      ...
      // If after 10 attempts we still can't connect to the network we need to
      // stop and break so we don't hang the device
      if (retry_count >= connectionRetries) {
        ERROR(F("Failed to connect to the access point after allotted tries! "
                "Is the network in range and are your credentials correct?"));
    ...
    while (WiFi.begin(wifi_name) != WL_CONNECTED) {
      snprintf(output, OUTPUT_SIZE,
               "Attempting to connect to AP (Attempt %i)...", retry_count + 1);
      LOG(output);
      delay(5000);
      retry_count++;
...
bool Loom_WIFI::verifyConnection() {
  ...
  if (moduleInitialized && powerUp) {
    ...
    if (pingLatency >= 0) {
      snprintf(output, OUTPUT_SIZE,
               "Successfully Pinged Google! Response Time: %ims", pingLatency);
      LOG(output);
      FUNCTION_END;
      return true;
    } else {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
bool Loom_WIFI::getNetworkTime(int *year, int *month, int *day, int *hour,
