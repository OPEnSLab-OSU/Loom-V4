...
public:
   ...
   * @param mode Whether or not to try to connect to an access point or create
   * our own
   ...
   */
  Loom_WIFI(Manager &man, CommunicationMode mode, const char *name = "",
            const char *password = "", int connectionRetries = 5);
  ...
  /**
   * Load the Wifi credentials from a JSON string, used to pull credentials from
   * a file
   *
  ...
  /**
   * Attempt to ping Google, this tests if we are truly connected to the
   * internet
   */
  ...
  /* Take in new WiFi information store it to flash and restart the WiFi chip to
   * put the new credentials into effect*/
...
private:
  ...
  WiFiClient wifiClient; // Wifi client that can be used with the MQTT client or
                         // other additional objects
  Loom_BatchSD *batchSD = nullptr;

  bool powerUp =
      true; // Whether or not the WiFi should power up (used with batch uploads)
