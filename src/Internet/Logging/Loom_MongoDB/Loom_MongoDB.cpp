...
Loom_MongoDB::Loom_MongoDB(Manager &man, NetworkComponent &internet_client,
                           ...
                           const char *database_name, const char *broker_user,
  ...
  if (moduleInitialized) {
    ...
    if (strlen(projectServer) > 0)
      ...
      snprintf_P(topic, MAX_TOPIC_LENGTH, PSTR("%s/%s/%s%i"), projectServer,
                 database_name, manInst->get_device_name(),
...
bool Loom_MongoDB::publish(Loom_BatchSD &batchSD) {
  ...
  if (moduleInitialized) {
    ...
    if (batchSD.shouldPublish()) {
      ...
      /* Utilize a stream so it doesn't matter how much data we have as its read
       * in one by one */
      while (fileOutput.available()) {
        c = fileOutput.read();

        // \r Marks the end of a line, at this point we want to publish that
        // whole packet
        if (c == '\r') {
