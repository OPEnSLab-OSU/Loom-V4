...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_ThingSpeak::Loom_ThingSpeak(Manager &man,
                                 NetworkComponent &internet_client,
                                 int channelID, const char *clientID,
...
bool Loom_ThingSpeak::publish() {
  ...
  if (moduleInitialized) {
    ...
    WARNING(F("Module not initialized! If using credentials from SD make sure "
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_ThingSpeak::addFunction(int fieldNumber,
                                  FloatReturnFuncDefs function) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_ThingSpeak::addFunction(int fieldNumber,
                                  FloatReturnFuncDefsWithParam function,
                                  int parameter) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_ThingSpeak::formatMessage(char topic[MAX_TOPIC_LENGTH],
                                    char message[MAX_JSON_SIZE]) {
  char tempBuffer[100];
  ...
  if (functionsNoParam.size() + functionsParam.size() > 8) {
    WARNING(
        F("There have been more than 8 fields added. ThingSpeak only supports "
          "up to 8 fields so any fields after the initial 8 will be ignored"));
  }

  /* Use the same variable for both loops, first looping over the list of
   * functions with no parameters, also check if we are still less than 8 fields
   */
  int i;
  ...
  /* Next do the same thing except with list of functions that have a single
   * integer paramater, also check if we are still less than 8 fields*/
  ...
  /* Check if we have a timestamp property if so we want to add a "created_at"
   * paramater to the message that we are publishing */
  if (!manInst->getDocument()["timestamp"].isNull()) {
    snprintf(
        tempBuffer, 100, "created_at=%s&",
