...
/* Define function signatures for functions of type "float name()" and "float
 * name(int parameter)"*/
...
public:
   ...
   * @param internet_client Reference to whatever connectivity platform is being
   * used
  ...
  Loom_ThingSpeak(Manager &man, NetworkComponent &internet_client,
                  int channelID, const char *clientID, const char *broker_user,
  ...
  /**
   * Add a new function to the list of functions that we are going to pass into
   * ThingSpeak
   *
   ...
   */
  void addFunction(int fieldNumber, FloatReturnFuncDefsWithParam function,
                   int parameter);
...
private:
   ...
   * Example:
   * field1=343&field2=421.4&created_at=2023-02-21T11:46:51Z&status=MQTTPUBLISH
   *
   * @param topic The topic buffer we should format to publish data to our given
   * feed
