...
protected:
   ...
   * @param retain Whether or not we want to the message to be retained on the
   * specified topic (default = false)
   * @param qos What quality-of-service we want to upload the message with
   * (default = 2)
   *
   * @return The status of the publish attempt
   */
  bool publishMessage(const char *topic, const char *message,
                      bool retain = false, int qos = 2);

  /**
   * Subscribe to a given topic to get the retained message and then immediately
   * unsubscribe
   *
  ...
  /**
   * Publishes a message to a given topic with size 0 and the retain flag set to
   * true so that the current retained message is removed
   *
  ...
  /**
   * Wrapper for specifying that we want to use a clean session or not (defaults
   * to true)
   *
  ...
  /* On power up we want to connect to the broker, this can be overridden but
   * you will need to call connectToBroker again */
  virtual void power_up() override { connectToBroker(); };

  /* On initialize we just want to call the power_up function to connect to the
   * broker, this can also be overriden but you should call power_up or
   * connectToBroker again */
...
public:
  ...
  /**
   * Load the MQTT credentials from a JSON string, used to pull credentials from
   * a file
   * @param jsonString JSON formatted string containing the login credentials,
   * this is freed at the end
   */
...
private:
  ...
  int keep_alive = 60000; // How long the broker should keep the connection
                          // open, defaults to a minute
