...
public:
   ...
   * @param wifi Reference to the Wifi manager for getting UDP communication
   * streams
   * @param mode How traffic is handled between the feather and the max client
   * (CLIENT = Remote Router, AP = Access point on the feather)
   */
  ...
  Loom_Max(Manager &man, Loom_WIFI &wifi, T *firstAct,
      ...
      : Module("Max Pub/Sub"), manInst(&man), wifiInst(&wifi) {
    get_variadic_parameters((Actuator *)firstAct,
...
private:
  ...
  std::vector<Actuator *>
      actuators; // List of actuators we want to control with max
