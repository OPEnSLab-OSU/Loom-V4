...
bool Loom_Max::publish() {
  ...
  if (actuators.size() > 0) {
    for (int i = 0; i < actuators.size(); i++) {
      actuators[i]->package(
          manInst->get_data_object(actuators[i]->getModuleName()));
    }
  }
  ...
  if (size <= 0) {
    ERROR(F("An error occurred when attempting to write the JSON packet to the "
            "UDP stream"));
    return false;
  }
  ...
  if (udpRecv->parsePacket()) {
    ...
    if (error != DeserializationError::Ok) {
      snprintf(output, OUTPUT_SIZE,
               "Failed to parse JSON data from UDP stream, Error: %s",
               error.c_str());
      ERROR(output);
      return false;
    }

    // If there are actuators supplied control those if not just print the
    // packet
    if (actuators.size() > 0) {
      ...
      if (strcmp(messageJson["type"].as<const char *>(), "command") == 0) {
        ...
        for (int j = 0; j < messageJson["commands"].as<JsonArray>().size();
             j++) {

          // Loop over each actuator to find the right one
          const char *type =
              messageJson["commands"][j]["module"].as<const char *>();
          ...
          for (int i = 0; i < actuators.size(); i++) {
            ...
            if (strcmp(actuators[i]->typeToString(), type) == 0) {
              ...
              // If the type we are trying to control doesn't have an instance
              // number
              if (strstr(type, "Relay") != NULL ||
                  strstr(type, "Neopixel") != NULL) {
                actuators[i]->control(
      ...
      if (strstr(messageJson["commands"][0]["module"].as<const char *>(),
                 "MaxSub") != NULL) {

        // Then we are trying to set the WiFi credentials
        if (messageJson["commands"][0]["func"].as<int>() == 99) {

          // Get the name and password out of the parameters and then power
          // cycle the board
          const char *name =
              messageJson["commands"][0]["params"][0].as<const char *>();
          const char *password =
