...
//////////////////////////////////////////////////////////////////////////////////////////////////////
Loom_RemoteManager::Loom_RemoteManager(Manager &man,
                                       NetworkComponent &internet_client,
                                       const char *broker_address,
                                       int broker_port, const char *broker_user,
...
bool Loom_RemoteManager::publish() {
  // Create topic name buffer and message buffer as well as a temp JSON document
  // to parse the received packets into
  ...
  // Check if we are using a hypnos and then update the parameters about the
  // hypnos
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_RemoteManager::updateHypnosInterval(
    char topic[MAX_TOPIC_LENGTH], char message[MAX_JSON_SIZE],
  ...
  /*
      ...
      The packet published here by the remote management interface should be as
     follows
      {
          "days": 0,
          "hours": 0,
  ...
  */
  snprintf(topic, MAX_TOPIC_LENGTH,
           "RemoteManager/%s%i/Hypnos/setSleepInterval",
           manInst->get_device_name(), manInst->get_instance_num());
  if (getCurrentRetained((const char *)topic, message)) {

    // Parse the incoming message into a JSON Document and then create a new
    // date time from the values to update the current time in the Hypnos
    deserializeJson(json, (const char *)message);
    TimeSpan time =
        TimeSpan(json["days"].as<int>(), json["hours"].as<int>(),
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Loom_RemoteManager::updateHypnosTime(
    char topic[MAX_TOPIC_LENGTH], char message[MAX_JSON_SIZE],
    StaticJsonDocument<MAX_JSON_SIZE> &json) {
  // Clear message and topic and json
  memset(topic, '\0', MAX_TOPIC_LENGTH);
  ...
  /*
      This is the topic that we need to publish to to change the RTC time. The
     contents of the packet can be whatever there just needs to be a packet
  */
...
void Loom_RemoteManager::updateDeviceStatus(bool onOff) {
  ...
  snprintf(topic, MAX_TOPIC_LENGTH, "RemoteManager/%s%i/status",
           manInst->get_device_name(), manInst->get_instance_num());
