...
class Loom_RemoteManager : public MQTTComponent {
  ...
  public:
     ...
     */
    Loom_RemoteManager(Manager &man, NetworkComponent &internet_client, const char *broker_address,
                       int broker_port, const char *broker_user = "", const char *broker_pass = "");

    /**
     * Barebones Constructor you will need to use loadConfigFromJSON to set the
     * credentials
     *
  ...
  private:
    ...
    /* Hypnos */
    void updateHypnosInterval(char topic[MAX_TOPIC_LENGTH], char message[MAX_JSON_SIZE],
                              StaticJsonDocument<MAX_JSON_SIZE> &json);
    void updateHypnosTime(char topic[MAX_TOPIC_LENGTH], char message[MAX_JSON_SIZE],
                          StaticJsonDocument<MAX_JSON_SIZE> &json);
};
