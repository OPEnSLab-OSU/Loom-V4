...
public:
  ...
  /**
   * Registers a new sub-module to be controlled by the manager (Used on sensors
   * so measure and package calls can all be called at once)
   * @param module Pointer to a class the inherits from Module that we want to
   * add
   */
   ...
   */
  DynamicJsonDocument &
  getDocument(); // Returns a reference to the main JSON document storing

  /**
   * Add a random piece of data to the overall JSON package in the given module
   * name with a name for the data
  ...
  /**
   * Start the Serial interface with some parameters, should we wait up to 20
   * seconds for the serial interface to open before continuing
   * @param waitForSerial Whether or not we should wait 20 seconds for the user
   * to open the serial monitor before continuing
   */
  ...
  /**
   *  Calls the measure function to pull data from the sensors on all added
   * modules
   */
  void measure();

  /**
   *  Calls the package function to store all data from those sensors into a
   * nice JSON package
   */
  void package();
...
private:
  ...
  void
  read_serial_num(); // Read the serial number out of the feather's registers

  /* Module Data */
  DynamicJsonDocument
      doc; // JSON document that will store all sensor information
  ...
  /* Validation */
  bool hasInitialized =
      false; // Whether or not the initialize function has been called, if not
             // it could be the source of hanging so we want to know
  bool usingHypnos = false;   // If the setup is using a hypnos
  bool hypnosEnabled = false; // If the power rails on the hypnos are enabled
                              // this means we should be able to initialize
};