...
// relevant function.
#define INSTRUMENT()                                                           \
  FunctionInstrumentor _instrumentor##__LINE__(__FILE__, __func__, __LINE__);
...
public:
   ...
   * @param manager Pointer to the SD manager to allow us to utilize SD logging
   * functionality
   */
  void setSDManager(SDManager *manager) { sdInst = manager; };

  /**
   * Set the instance of the Hypnos, this should be used if you want the current
   * timestamp added to the front of the logger output
  ...
  void genericLog(LogContext log, const char *msg) {
    ...
    if (hypnosInst != nullptr && hypnosInst->isRTCInitialized()) {
      snprintf_P(logMessage, OUTPUT_SIZE, PSTR("[%s] [%s] [%s:%s:%u] %s"),
                 hypnosInst->getCurrentTime().text(), log.level, fileName,
                 log.func, log.lineNum, msg);
    } else {
      snprintf_P(logMessage, OUTPUT_SIZE, PSTR("[%s] [%s:%s:%u] %s"), log.level,
