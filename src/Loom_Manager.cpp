...
void Manager::registerModule(Module *module) {
  ...
  for (int i = 0; i < modules.size(); i++) {
    ...
    // Check if the module name contains the base string to make sure this works
    // past 2 modules of the same type
    if (location != NULL) {
      // Append the address to the name
      char modifiedName[100];

      // Format first module name
      snprintf_P(modifiedName, 100, PSTR("%s_%i"),
                 modules[i].second->getModuleName(),
      ...
      // Once we find a module of this type we want to break out to avoid
  ...
  if (hasInitialized) {
    ...
    for (int i = 0; i < modules.size(); i++) {
      if (modules[i].second->moduleInitialized)
        ...
        snprintf(noInitLog, 50, "%s Not initialized!",
                 modules[i].second->getModuleName());
        WARNING(noInitLog);
      }
      TIMER_RESET;
    ...
    ERROR(F("Unable to collect data as the manager and thus all sensors "
            "connected to it have not been initialized! Call "
            "manager.initialize() to fix this."));
  }
...
void Manager::package() {
  ...
  // Clear the document so that we don't get null characters after too many
  // updates
  doc.clear();
  doc[F("type")] = F("data");
...
JsonObject Manager::get_data_object(const char *moduleName) {
  ...
  for (JsonVariant value : contentsArray) {
    ...
    if (strcmp(value.as<JsonObject>()["module"].as<const char *>(),
               moduleName) == 0) {
...
void Manager::power_up() {
  ...
  // If we didn't already disable the timer from finding the LTE we should
  // disable it now
  Watchdog.disable();
  FUNCTION_END;
}
...
void Manager::initialize() {
  FUNCTION_START;
  // If you are using a hypnos board that has not been enabled, this needs to
  // occur before initializing sensors
  if (usingHypnos && !hypnosEnabled) {
    LOG(F("Your sketch is set to use a Hypnos board which has not been enabled "
          "before attempting to initialize sensors. \nThis will causing "
          "hanging please enable the board before initialization. Continuing "
          "but know this may cause issues!"));
  }
