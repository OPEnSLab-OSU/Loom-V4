...
class Loom_Heartbeat {
  public:
     ...
     *
     *         // PERSONAL_NOTE: Might not need manager reference, but haven't fully commited to
     * taking it out yet.
     *
     ...
     * taking it out yet.
     */
    Loom_Heartbeat(const uint32_t pHeartbeatInterval, const uint32_t pNormalWorkInterval,
                   Manager *managerInstance, Loom_Hypnos *hypnosInstance = nullptr);
  ...
  private:
    uint32_t heartbeatTimer_s =
        0; // internally tracked timer to keep track of remaining time until next heartbeat
    uint32_t heartbeatInterval_s =
        0; // user defined interval between heartbeats (doesn't get adjusted)
