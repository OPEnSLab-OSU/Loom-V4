...
| [#301 — LTE network time 15 hours behind](https://github.com/OPEnSLab-OSU/Loom-V4/issues/301) | **Addressed.** The LTE time path normalizes TinyGSM's timezone direction and converts modem-local time back to UTC before Hypnos updates the DS3231. Network-time methods initialize outputs and return their real success status. | `src/Internet/Connectivity/Loom_LTE/Loom_LTE.cpp`; `src/Hardware/Loom_Hypnos/Loom_Hypnos.cpp` |
| [#302 — SmartRock first-wake freeze](https://github.com/OPEnSLab-OSU/Loom-V4/issues/302) | **Targeted; full stack validation remains in progress.** The Hypnos stale-alarm and interrupt fixes address the first-wake deadlock path and pass the standalone replacement-alarm test. SmartRock now passes sketch compile time, handles nested SD sleep configuration, allows rail settling, and uses corrected MS5803 initialization. | `src/Hardware/Loom_Hypnos/`; `src/Sensors/I2C/Loom_MS5803/`; `examples/Lab Examples/SmartRock/` |
...
- Corrected LTE network-time conversion to UTC and initialized every time output before use.
